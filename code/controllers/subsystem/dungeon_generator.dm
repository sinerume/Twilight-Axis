...
#define DUNGEON_MIN_INITIAL_HELPERS 4
...
/datum/controller/subsystem/dungeon_generator/proc/find_initial_map_data()
	...
	if(setup_done || generation_complete)
		return

	var/list/helpers_by_z = list()
	for(var/obj/effect/dungeon_directional_helper/H in world)
		if(QDELETED(H))
			continue

		var/z_key = "[H.z]"
		if(!helpers_by_z[z_key])
			helpers_by_z[z_key] = list()
		helpers_by_z[z_key] += H
		CHECK_TICK

	var/list/found_points = list()
	if(target_z)
		found_points = helpers_by_z["[target_z]"]
	else
		var/best_z = 0
		var/best_count = 0
		for(var/z_key in helpers_by_z)
			var/list/z_points = helpers_by_z[z_key]
			var/z_count = length(z_points)
			if(z_count > best_count)
				best_z = text2num(z_key)
				best_count = z_count
				found_points = z_points
			CHECK_TICK

		if(best_z)
			target_z = best_z

	if(length(found_points) < DUNGEON_MIN_INITIAL_HELPERS)
		target_z = 0
		setup_attempts++
	...
	for(var/i in 1 to DUNGEON_MIN_INITIAL_HELPERS)
...
/datum/controller/subsystem/dungeon_generator/proc/try_bridge_gap(obj/effect/dungeon_directional_helper/helper, turf/target_turf)
	if(!target_turf || !istype(target_turf, /turf/closed/wall/mineral/rogue))
		return FALSE
...
/datum/controller/subsystem/dungeon_generator/proc/scan_free_area(turf/start_T, dir)
	...
	for(var/i in 1 to 30)
		...
		if(!T || !is_strictly_void(T) || is_protected(T.x, T.y))
			break
		depth = i
		CHECK_TICK
	return list("h" = depth)
...
/datum/controller/subsystem/dungeon_generator/proc/can_place(datum/map_template/dungeon/T, turf/start_T)
	...
	if(ex > world.maxx || ey > world.maxy || start_T.x < 1 || start_T.y < 1)
		return FALSE
	...
	for(var/z_off in 0 to 1)
		...
		for(var/turf/test in block(locate(start_T.x, start_T.y, cz), locate(ex, ey, cz)))
			if(z_off == 0)
				if(!is_strictly_void(test) || is_protected(test.x, test.y))
					return FALSE
			else
				if(test.density && !is_strictly_void(test))
					return FALSE

			for(var/obj/O in test)
				if(istype(O, /obj/effect/dungeon_directional_helper))
					continue
				if(O.density)
					return FALSE
			CHECK_TICK
...
#undef STAGE_CLEANUP
#undef DUNGEON_MIN_INITIAL_HELPERS
